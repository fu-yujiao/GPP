`GAPIT.Multiple.Manhattan`<-function(model_store,DPP=50000,chor_taxa=NULL,cutOff=0.01,band=5,seqQTN=NULL,byTraits=FALSE,
    Y.names=NULL,GM=NULL,interQTN=NULL,WS=10e5,outpch=NULL,inpch=NULL,
    plot.style="Oceanic",plot.line=TRUE,plot.type=c("h","s","w")){
#model_store= c("FarmCPU", "BLINK")  
#Y.names= c("EarHT", "dpoll") 
...
store.x=NULL
y_filter0=NULL
result0 <- data.frame()  # 初始化为空数据框
y_filter0 <- data.frame()  # 初始化为空数据框
...
if(length(sig_pos[!is.na(sig_pos)])>1)
 ...
 #       x_matrix=cbind(as.data.frame(rownames(x_matrix)),x_matrix)
       #print(x_matrix)
        lastbase=0
        map_store=cbind(as.data.frame(environ_result[,2]),as.numeric(environ_result[,3]))
        ticks=NULL
...
if("h"%in%plot.type){
	...
	if(k==Nenviron)
		...
		file_path <- paste("GAPIT.Association.GWAS_Results.", environ_name[k], ".csv", sep = "")
        cat("正在处理文件:", file_path, "\n")
             if (!file.exists(file_path)) {
             cat("文件不存在:", file_path, "\n")
             next
             }
       try({
       environ_result=read.csv(file_path,head=T)
	   cat("数据读取成功，维度为:", dim(environ_result), "\n")
	   result=environ_result[,1:4]
       result=result[order(result[,3]),]
       result=result[order(result[,2]),]
       cat("排序后数据维度:", dim(result), "\n")
       rownames(result)=1:nrow(result)
       GI.MP=result[,c(2:4)]
	   cat("GI.MP 初始维度:", dim(GI.MP), "\n")
       borrowSlot=4
    ...
    #Retain SNPs that have P values between 0 and 1 (not na etc)
	   ...
	   cat("过滤后 GI.MP 维度:", dim(GI.MP), "\n")
      #}, silent = TRUE)
    #Remove chr 0 and 99
       GI.MP <- GI.MP[GI.MP[,1]!=0,]
	   print(dim(GI.MP))  # 查看数据行列数
    ...
    # print(dim(GI.MP))
       ...
       cat("正在绘制环境:", environ_name[k], "\n")
       cat("标记的数量:", numMarker, "\n")
       cat("显著标记的数量:", nrow(GI.MP), "\n")
	   }, silent = TRUE)}
	#end of environ_name
       dev.off()
}#end of plot.type
...
if("w"%in%plot.type){
 ...
 box()
 }, silent = TRUE)}#end of environ_name
 dev.off()
}#end of plot.type
...
if("s"%in%plot.type)
{
 ...
 for(k in 1:Nenviron)
  { 
    ...
    result=result[order(result[,3]),]
    result=result[order(result[,2]),]
    result=result[match(as.character(environ_result[,1]),as.character(result[,1])),]
    rownames(result)=1:nrow(result)
    GI.MP=result[,c(2:4)]
    borrowSlot=4
    GI.MP[,borrowSlot]=0 #Inicial as 0
